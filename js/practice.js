  ...
  function customEditor(app, editing) {
    ...
    card.innerHTML = `
      ...
      <div class="tips">${editing
        ...
        : `붙여 넣기는 <b>Ctrl + V</b> (맥은 <b>⌘ + V</b>)예요. <b>이 컴퓨터에만 저장</b>은 나만 보고, <b>모두에게 공유하기</b>는 이 사이트에 들어오는 모든 사람의 긴글연습 목록에 바로 올라가요. ${me ? '로그인한 채로 올린 글은 나중에 내 계정으로 언제든 고치거나 지울 수 있어요. 비밀번호는 다른 컴퓨터에서 로그인 없이 지우고 싶을 때만 필요해요.' : '공유한 글은 올릴 때 정한 비밀번호 4자리로 고치거나 지울 수 있어요.'}`}</div>`;
    app.appendChild(card);
    const body = $('#my-body');
