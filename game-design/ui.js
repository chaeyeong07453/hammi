...
export function mountGame(root, { game='race', state, onEvent=()=>{} } = {}) {
 ...
 const renderPlayers = () => current.players.map(p=>`<div class="gd-competitor ${p.id==='me'?'is-me':''} ${game==='tennis'&&current.turnId&&p.id===current.turnId&&['playing','urgent','waiting','invalid','wrong-start','duplicate','timeout'].includes(current.phase)?'is-turn':''}">${avatar(p.color,p.name)}<div><strong><span class="gd-player-name" title="${esc(p.name)}">${esc(p.name)}</span> ${p.id==='me'&&p.name!=='나'?'<span class="gd-me-tag">나</span>':''}</strong><span>${game==='race'?esc(p.subtitle):hearts(p.lives)}</span></div>${game==='race'?`<b>${esc(p.score)}<small>점</small></b>`:''}</div>`).join(game==='tennis'&&current.players.length===2?'<span class="gd-vs">VS</span>':'');
 function render() {
  if(disposed)return;
  ...
  $('matchbar').innerHTML=`<div class="gd-competitors">${renderPlayers()}</div><div class="gd-timer ${current.phase==='urgent'?'is-urgent':''}">${icon('clock')}<div><span>${game==='race'?'남은 시간':current.turn==='me'?'받아칠 시간':(current.players.length>2&&current.turnName?esc(current.turnName)+'님의 차례':'상대의 차례')}</span><strong>${game==='race'?time(current.remaining):current.seconds+'<small>초</small>'}</strong></div></div>`;
  if(game==='race') {
   ...
   $('courtword').innerHTML=`<span class="gd-turn-pill">${icon(current.turn==='me'?'racket':'clock')} ${current.turn==='me'?'내 차례예요':(current.players.length>2&&current.turnName?esc(current.turnName)+'님 차례예요':'상대의 차례예요')}</span><div class="gd-last-word"><span>${current.lastById==='me'?'내가 보낸 단어':current.lastByName?esc(current.lastByName)+'님이 보낸 단어':current.turn==='me'?'상대가 보낸 단어':'새로 온 공'}</span><strong>${esc(last.slice(0,-1))}<em>${esc(last.slice(-1))}</em></strong>${icon('arrow')}<b>${esc(current.startLetter)}</b></div>${last&&meanings[last]?`<div class="gd-word-meaning">${icon('book')} ${esc(meanings[last])}</div>`:''}`;
   $('words').innerHTML=['me','opponent'].map(k=>{const p=k==='me'?current.players.find(x=>x.id==='me'):(current.foe||current.players.find(x=>x.id!=='me'));return p?`<div class="gd-bear-label gd-bear-${k}" data-target="${k}" style="visibility:hidden">${avatar(p.color,p.name)}<strong>${esc(p.name)}</strong>${k==='me'?'<span class="gd-me-tag">나</span>':''}</div>`:'';}).join('');
   $('side').innerHTML=`<section class="gd-side-card"><div class="gd-side-title">${icon('racket')}<h2>이어지는 랠리</h2><b class="gd-rally-count">${esc(current.rally)}<small>회</small></b></div><p class="gd-side-sub">한 단어씩, 여기까지 왔어요.</p><ol class="gd-chain">${current.chain.slice(-5).map((word,i,arr)=>`<li class="${i===arr.length-1?'is-latest':''}"><span class="gd-chain-dot"></span><span class="gd-chain-word"><span>${esc(word.slice(0,-1))}<b>${esc(word.slice(-1))}</b></span>${meanings[word]?`<em class="gd-chain-meaning">${esc(meanings[word])}</em>`:''}</span><small>${(()=>{const by=(current.chainBy||[])[current.chain.length-arr.length+i];return i===arr.length-1?'방금 전':by!=null&&by!==''?esc(by):(by===''?'새 공':(i%2?'나':'상대'));})()}</small></li>`).join('')}</ol><div class="gd-next-letter"><span>다음 시작 글자</span><strong>${esc(current.startLetter)}</strong>${icon('arrow')}</div></section><section class="gd-side-card gd-tennis-rules"><h3>${icon('book')} 함께 지키는 약속</h3><p>실제로 있는 낱말만 사용해요.</p><p>한 번 쓴 단어는 다시 쓰지 않아요.</p><p>놓친 공 하나에 생명 하나가 줄어요.</p></section>`;
  }
